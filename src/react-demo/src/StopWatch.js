...
const StopWatch = observe(
  ...
  () => {
    ...
    const time$ = button.pipe(switchMap(value => {
      switch(value) {
        case 'start': {
          return interval(10).pipe(timeInterval(), scan((result, ti) => result + ti.interval, 0));
        }
        case 'stop': {
          return empty();
        }
    ...
    return StopWatch.pipe(merge(time$), map(value => ({
      milliseconds: value,
